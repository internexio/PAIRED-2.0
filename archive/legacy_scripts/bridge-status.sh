...
show_json_status() {
  ...
  if [[ "$service_path" == "$LOCAL_PAIRED_DIR"* ]]; then
    service_source="local"
  fi
  
  if [[ "$client_path" == "$LOCAL_PAIRED_DIR"* ]]; then
    client_source="local"
  fi
  
  if [[ "$api_path" == "$LOCAL_PAIRED_DIR"* ]]; then
    api_source="local"
  fi
...
show_detailed_status() {
  ...
  if is_bridge_running; then
    ...
    if [ -f "$LOCAL_PAIRED_DIR/src/cascade_bridge_service.js" ]; then
    ...
    if [ -f "$LOCAL_PAIRED_DIR/src/cascade_bridge_client.js" ]; then
    ...
    if [ -f "$LOCAL_PAIRED_DIR/src/cascade_bridge_api.js" ]; then
    ...
    # Show summary
    local local_count=$(find "$LOCAL_PAIRED_DIR/src" -name "cascade_bridge_*.js" 2>/dev/null | wc -l)
    
    if [ -d "$LOCAL_PAIRED_DIR/src" ] && [ "$local_count" -gt 0 ]; then
...
show_path_info() {
  echo -e "${CYAN}${BOLD}PAIRED Bridge File Paths${NC}"
  echo -e "${BLUE}=======================${NC}\n"
  
  # Check if local .paired directory exists
  if [ -d "$LOCAL_PAIRED_DIR" ]; then
    echo -e "${CYAN}Local project directory:${NC} $LOCAL_PAIRED_DIR"
  else
    echo -e "${YELLOW}⚠️ No local .paired directory found${NC}"
  fi
  
  echo -e "${CYAN}Global installation:${NC} $GLOBAL_PAIRED_DIR\n"
  ...
  # Show summary
  echo -e "\n${CYAN}Summary:${NC}"
  local_count=$(find "$LOCAL_PAIRED_DIR/src" -name "cascade_bridge_*.js" 2>/dev/null | wc -l)
  global_count=$(find "$GLOBAL_PAIRED_DIR/src" -name "cascade_bridge_*.js" 2>/dev/null | wc -l)
...
show_message_queue() {
  ...
  echo -e "${CYAN}=========================================${NC}"
  echo -e "${CYAN}      PAIRED Cascade Bridge Message Queue    ${NC}"
  echo -e "${CYAN}=========================================${NC}"
...
copy_local() {
  ...
  mkdir -p "$LOCAL_PAIRED_DIR/src"
  mkdir -p "$LOCAL_PAIRED_DIR/cascade_bridge"
  
  # Copy bridge files
  echo -e "${BLUE}⏳ Copying bridge files to local project...${NC}"
  cp "$GLOBAL_PAIRED_DIR/src/cascade_bridge_service.js" "$LOCAL_PAIRED_DIR/src/" 2>/dev/null && echo -e "${GREEN}✓ Copied cascade_bridge_service.js${NC}" || echo -e "${RED}✗ Failed to copy cascade_bridge_service.js${NC}"
  cp "$GLOBAL_PAIRED_DIR/src/cascade_bridge_client.js" "$LOCAL_PAIRED_DIR/src/" 2>/dev/null && echo -e "${GREEN}✓ Copied cascade_bridge_client.js${NC}" || echo -e "${RED}✗ Failed to copy cascade_bridge_client.js${NC}"
  cp "$GLOBAL_PAIRED_DIR/src/cascade_bridge_api.js" "$LOCAL_PAIRED_DIR/src/" 2>/dev/null && echo -e "${GREEN}✓ Copied cascade_bridge_api.js${NC}" || echo -e "${RED}✗ Failed to copy cascade_bridge_api.js${NC}"
  cp "$GLOBAL_PAIRED_DIR/src/cascade_bridge_lifecycle.js" "$LOCAL_PAIRED_DIR/src/" 2>/dev/null && echo -e "${GREEN}✓ Copied cascade_bridge_lifecycle.js${NC}" || echo -e "${RED}✗ Failed to copy cascade_bridge_lifecycle.js${NC}"
  cp "$GLOBAL_PAIRED_DIR/src/cascade_bridge_client_auto.js" "$LOCAL_PAIRED_DIR/src/" 2>/dev/null && echo -e "${GREEN}✓ Copied cascade_bridge_client_auto.js${NC}" || echo -e "${RED}✗ Failed to copy cascade_bridge_client_auto.js${NC}"
  cp "$GLOBAL_PAIRED_DIR/src/cascade_bridge_integration.js" "$LOCAL_PAIRED_DIR/src/" 2>/dev/null && echo -e "${GREEN}✓ Copied cascade_bridge_integration.js${NC}" || echo -e "${RED}✗ Failed to copy cascade_bridge_integration.js${NC}"
...
# Show usage
show_usage() {
  echo -e "${CYAN}${BOLD}PAIRED Bridge Status and Control${NC}"
