...
echo "🚀 Resuming Windsurf work on $MACHINE..."
...
# Check if .windsurf directory exists and show context
if [[ -d .windsurf ]]; then
    echo ""
    ...
    if [[ -f .windsurf/contexts/current_focus.md ]]; then
        echo "📍 Current Focus:"
        head -10 .windsurf/contexts/current_focus.md
        echo ""
    fi
    
    # Show recent AI memory entries
    if [[ -f .windsurf/memory/ai_memory.md ]]; then
        echo "🧠 Recent AI Memory:"
        tail -20 .windsurf/memory/ai_memory.md | head -10
        echo ""
    fi
fi

echo "✅ Resume complete. Ready for development!"
echo "💡 Tip: Source aliases with: source .windsurf/scripts/aliases.sh"
